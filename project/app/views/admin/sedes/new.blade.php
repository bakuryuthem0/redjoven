...
<div class="container">
    ...
    <div class="col-xs-12 formulario">
      <div class="col-xs-12 bg-info">
		...
		<form class="" method="POST" action="{{ URL::to('administrador/nueva-sede/enviar') }}" enctype="multipart/form-data">
      <div class="col-xs-12 input-group no-padding">
        <div class="formulario col-xs-12 col-md-6">
          ...
          <input type="text" name="name" class="form-control" placeholder="Nombre de la Sede" required value="{{ Input::old('name') }}">
          @if($errors->has('name'))
        ...
        <div class="formulario col-xs-12 col-md-6">
          ...
          <select name="coord" class="form-control">
            ...
            @foreach($users as $u)
              <option value="{{ $u->id }}" @if(Input::old('coord') == $u->id) selected @endif>{{ ucfirst(strtolower($u->username)) }}</option>
            @endforeach
          </select>
          @if($errors->has('coord'))
